...
public:
	FaceDetector(const string& model_dir,
		const MODEL_VERSION model_version, int GPUID);

	vector< BoundingBox > Detect(const cv::Mat& img, const COLOR_ORDER color_order, const IMAGE_DIRECTION orient, int min_size = 20, float P_thres = 0.6, float R_thres = 0.7, float O_thres = 0.7, float nms_thresh = 0.7f, bool is_fast_resize = true, float scale_factor = 0.709);
