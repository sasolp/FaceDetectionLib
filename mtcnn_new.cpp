...
FaceDetector::FaceDetector(const string& model_dir, const MODEL_VERSION model_version, int GPUID)
{
	this->model_version = model_version;
	if (GPUID < 0)
		Caffe::set_mode((Caffe::Brew)0);
	else
	{
 ...
 */
vector< FaceDetector::BoundingBox > FaceDetector::Detect(const cv::Mat& img, const COLOR_ORDER color_order, const IMAGE_DIRECTION orient, int min_size, float P_thres, float R_thres, float O_thres, float nms_thresh, bool is_fast_resize, float scale_factor)
{
    /*change image format*/
    cv::Mat sample;
    ...
    if(totalBoxes.size() > 0)
    {
        ...
        nms_cpu(filterOutBoxes, nms_thresh, UNION, totalBoxes);
    }
#ifdef IMAGE_DEBUG
    //debug
    tmp = img.t();
    ...
    if(totalBoxes.size() > 0)
    {
        ...
        nms_cpu(filterOutBoxes, nms_thresh, MIN, totalBoxes);
    }
#ifdef IMAGE_DEBUG
    //debug
    tmp = img.t();
...
void readFileList(const char* inputfolder, const char* ListFile, vector<string>& imgFiles)
{

	FILE *fp = fopen(ListFile, "r");
	char temppath[_MAX_FNAME];
	while (!feof(fp))
	{
		fscanf(fp, "%s", &temppath[0]);
		char *filepath = (char *)malloc(_MAX_FNAME * sizeof(char));
		sprintf(filepath, "%s\\%s", inputfolder, temppath);
		imgFiles.push_back( filepath);
	}
	fclose(fp);
}
...
int main(int argc, char **argv) {
   ...
   /* FaceDetector fd("FaceDetectionModels", FaceDetector::MODEL_V1,-1);
    
    vector<string> imgList;
    readFileList(".","List.txt", imgList);
